function [vlast,wlast,vsend,wsend,v_movx,v_movy,stateRobo,stateObst,oth,r_ob]=launch_iros(x0,y0,x_ob,y_ob,theta_in,na,mela,ob)
...
tic
[w_guess,v_guess,delt,x_points_left,y_points_left,x_points_right,y_points_right,wp,n_w,n_ob,v_movx,v_movy,r_ob,n,avg_speed,oth] = get_input(x0,y0);
mred=0;
x_in=x0;obinx = x_ob;
y_in=y0;obiny = y_ob;
thet = theta_in;
...
ful= sum(fin)+0.1*(sum(slack1.^2)+sum(slack2.^2));
minimize(ful) 
subject to 
if (road_bound~=0)
lsum==0;
...
-2.0*delt <= w_cvx(2:n) - w_cvx(1:(n-1))<= 2.0*delt;
...
if (abs(preval-cvx_optval)<=0.5 )  %tolerance corection 
    ct=0;
else
   ct=ct+1;
end
...
n_w=n_w-red;
vsend=[vsend,vel(1:na)];
wsend=[wsend,w_cvx(1:na)];
...
for tick =1:size(x_ob,2)
obroadx=[obroadx,xob(tick,(1:red))];
obroady=[obroady,yob(tick,(1:red))];
end
mred = mred+red;
if (mred>=na)
    break
...
stateRobo=[x_in,y_in,thet,vsend(1),wsend(1)];
