function [vlast,wlast,vsend,wsend,v_movx,v_movy,stateRobo,stateObst,oth,r_ob,x1,y1]=launch_iros(x0,y0,x_ob,y_ob,theta_in,na,mela,ob,vipas,wipas)
...
vipas=double(vipas);wipas=double(wipas);
tic
[w_guess,v_guess,delt,x_points_left,y_points_left,x_points_right,y_points_right,wp,n_w,n_ob,v_movx,v_movy,r_ob,n,avg_speed,oth] = get_input(x0,y0,firstval);
mred=0;
x_in=x0;obinx = x_ob;
y_in=y0;obiny = y_ob;
thet = theta_in;
...
ful= sum(fin)+0.1*(sum(slack1.^2)+sum(slack2.^2))+cu1^2+cu2^2;
minimize(ful) 
subject to 
if (road_bound~=0)
lsum==0;
...
stateRobo=[x_in,y_in,thet,vipas,wipas];
