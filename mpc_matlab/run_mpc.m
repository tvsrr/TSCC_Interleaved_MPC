function [vlast,wlast,vsend,wsend,v_movx,v_movy,stateRobo,stateObst,oth,r_ob,cumx,cumy]=launch_iros(x0,y0,x_ob,y_ob,theta_in,na,mela,ob)
tic
...
stateObst(obi,:)=[obinx(obi),obiny(obi),oth(obi),v_movx(obi)];
 ...
 csvwrite('cumxm.csv',cumx);
csvwrite('cumym.csv',cumy);
toc
end 