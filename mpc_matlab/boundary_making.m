%road boundaries
function [ coff_arc_x, coff_arc_y, arc_param , x_even, y_even, x_arc_points, y_arc_points ] = boundary_making( x_points, y_points, no_of_ac_curves, debug_param_plot )
...
%arc_param is the linearly spaced arc lengths which sum up to toal length
%of the curve


delta_t = 2;
t_start = 0;
n_points = size(x_points,1);   %x_points refer to the manual way points given

temp_n = n_points-2;  %number of waypoints-2
temp_ones = ones(temp_n,1);  
temp_a = diag(4*temp_ones,0); %a diagonal matrix of 4's in the diagonal
temp_a = temp_a + diag(temp_ones(1:temp_n-1),-1); 
temp_a = temp_a + diag(temp_ones(1:temp_n-1),1);
%putting ones on the either side of the diagonal elements
temp_con = 6/(delta_t)^2;
temp_b_x = temp_con*(x_points(3:end) + x_points(1:end-2) - 2*x_points(2:end-1)); %what is this?
...
coff_splines_x =  [a_coff_x,b_coff_x,c_coff_x,d_coff_x]; %these are the coefficients of the curve for x
...
coff_splines_y =  [a_coff_y,b_coff_y,c_coff_y,d_coff_y];   %finally these are th coefficients of the curve for y
...
for i = 1:size(coff_splines_x,1)
    ...
    for j = 1:size(temp_t,1)
        ...
        x_spline_points = [x_spline_points;cur_x];  %after bisection ofmthe splines
    ...
    segment_lengths = [segment_lengths ; length_curve]; %lengths of the splines formed
...
arc_param = linspace(0,length_curve,no_of_ac_parm)';  %divison of the arc lengths %third variable
...
for i = 2:no_of_ac_parm-1
    ...
    temp_pss = integral(s_fun,t_pick,t_mid);  %arc length
...
coff_arc_x =  [a_coff_x,b_coff_x,c_coff_x,d_coff_x]; %first variable
...
coff_arc_y =  [a_coff_y,b_coff_y,c_coff_y,d_coff_y]; %second variable
